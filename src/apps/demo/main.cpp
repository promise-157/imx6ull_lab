...
int main(int argc, char *argv[]) {
  ...
  HardwareCenterPage *hwCenterApp = new HardwareCenterPage(&w);
  TerminalPage *terminalApp = new TerminalPage(&w);

  // 增量式注册应用模块，此时主窗口不再通过数字硬编码寻找应用
  w.registerApp(musicApp);
  w.registerApp(videoApp);
  w.registerApp(hwCenterApp);
  w.registerApp(terminalApp);

  // 初始化全局虚拟键盘机制，挂载事件过滤器
  a.installEventFilter(&VirtualKeyboard::instance());
